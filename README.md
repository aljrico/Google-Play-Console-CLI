...
`subscription-offers create` accepts either Google Play API `SubscriptionOffer` JSON, `gpc subscription-offers get --output json` shape, or basic flags for one free or paid-price phase across explicit regions. The `--package`, `--product-id`, `--base-plan-id`, and `--offer-id` flags override immutable IDs in JSON files, and output-only `state` is ignored because Play creates draft offers. Basic flags require at least one `--free-region` or `--price REGION:CURRENCY:UNITS[:NANOS]`, plus `--phase-duration`; `--phase-recurrence` defaults to 1. Use JSON for absolute or relative discounts, two-phase offers, targeting rules, and other-regions config. Google Play only supports offers under auto-renewing base plans. The JSON body must include regional configs and one or two phases; each phase needs one price mode per configured region (`price`, `absoluteDiscount`, `relativeDiscount`, or `free`). Phase patch commands fetch current offers and preserve untouched phases/regions; `batch-patch-phase-free` clears existing regional price, absolute discount, and relative discount fields for the targeted phase regions. Example:
