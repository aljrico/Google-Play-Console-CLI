...
`one-time-product-offers create` accepts either Google Play API `OneTimeProductOffer` JSON, `gpc one-time-product-offers get --output json` shape, or basic flags for one discounted offer with regional relative discounts. The `--package`, `--product-id`, `--purchase-option-id`, and `--offer-id` flags override immutable IDs in JSON files, and output-only `state` and `regionsVersion` are ignored. Basic flags require at least one `--relative-discount REGION:0.5`, set regional availability to available, and support optional `--start-time`, `--end-time`, `--redemption-limit`, and repeatable `--offer-tag`; use JSON for absolute discounts, no-override regions, and pre-order offers. The command uses Google's `batchUpdate` with `allowMissing=true`, but live create first verifies the offer does not already exist. The JSON body must include exactly one offer type (`discountedOffer` or `preOrderOffer`) and regional configs with one price mode per region (`absoluteDiscount`, `relativeDiscount`, or `noOverride`). Example:
