...
| `notify` | `notify` | N/A | tested | `gpc notify send` posts generic JSON webhook notifications with dry-run/confirm gating and redacted endpoint output; `gpc notify slack` posts Slack incoming webhook payloads through the same guarded delivery path. Additional service-specific adapters remain planned. |
