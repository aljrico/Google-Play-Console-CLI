...
| `finance` | `finance reports summarize` | Play financial reports via downloaded CSV/GCS exports | tested | `gpc finance reports summarize` summarizes downloaded Play financial report CSVs by transaction type and merchant currency amount. Programmatic GCS download remains planned. |
...
| `webhooks` | `notifications` | Pub/Sub and Real-time developer notifications | tested | `gpc notifications rtdn decode` decodes wrapped Pub/Sub push payloads containing Google Play real-time developer notifications. Pub/Sub setup automation, unwrapped push delivery, and pull variants remain planned. |
