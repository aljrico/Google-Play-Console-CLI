...
| `subscriptions` | `subscriptions` | `monetization.subscriptions`, `basePlans` | tested | `gpc subscriptions list`, `get`, and `batch-get` cover subscription inspection; `delete` applies guarded deletion for subscriptions that have never had a base plan published; `base-plan activate` and `deactivate` apply guarded base plan state changes. Broader subscription and price mutations remain planned. |
