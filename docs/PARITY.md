...
| `subscriptions` | `subscriptions` | `monetization.subscriptions`, `basePlans` | tested | `gpc subscriptions list`, `get`, and `batch-get` cover subscription inspection; `patch` applies guarded localized listing updates with live merge preflight; `delete` applies guarded deletion for subscriptions that have never had a base plan published; `base-plan activate`, `deactivate`, `batch-activate`, and `batch-deactivate` apply guarded base plan state changes. Broader subscription creation and price mutations remain planned. |
