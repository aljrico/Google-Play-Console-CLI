...
| `crashes` | `vitals` | Play Developer Reporting API | tested | `gpc vitals metric-set get --metric-set crash-rate` and `error-count` cover crash/error metric-set metadata, and `gpc vitals metric-set query` can query crash/error metric rows. Issue/report listing remains planned. |
