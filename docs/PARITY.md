...
| `pricing` | `pricing` | `monetization.convertRegionPrices`, product/subscription pricing APIs | tested | `gpc pricing convert-region-prices` calculates Play regional prices from an explicit source price, and subscription base-plan price migrations are covered under `subscriptions base-plan batch-migrate-prices`. Broader product and subscription direct price edits remain planned. |
