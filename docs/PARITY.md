...
| `subscriptions` | `subscriptions` | `monetization.subscriptions`, `basePlans` | tested | `gpc subscriptions list`, `get`, and `batch-get` cover subscription inspection; `patch` and `batch-patch-listings` apply guarded localized listing updates with live merge preflight; `delete` applies guarded deletion for subscriptions that have never had a base plan published; `base-plan delete` applies guarded deletion for draft-only base plans; `base-plan activate`, `deactivate`, `batch-activate`, and `batch-deactivate` apply guarded base plan state changes; `base-plan batch-migrate-prices` applies guarded regional legacy-price cohort migrations. Broader subscription creation and direct price edits remain planned. |
