...
| `subscriptions` | `subscriptions` | `monetization.subscriptions`, `basePlans` | tested | `gpc subscriptions list`, `get`, and `batch-get` cover subscription inspection; `create` applies guarded draft subscription creation from a typed JSON body or basic auto-renewing/prepaid/installments base-plan and restricted-country flags; `patch` and `batch-patch-listings` apply guarded localized listing updates with live merge preflight; `delete` applies guarded deletion for subscriptions that have never had a base plan published; `base-plan delete` applies guarded deletion for draft-only base plans; `base-plan activate`, `deactivate`, `batch-activate`, and `batch-deactivate` apply guarded base plan state changes; `base-plan batch-migrate-prices` applies guarded regional legacy-price cohort migrations; `base-plan batch-patch-prices` fetches current subscriptions and applies guarded direct base-plan regional price patches without adding missing regional config. Broader compliance flag builders remain planned. |
