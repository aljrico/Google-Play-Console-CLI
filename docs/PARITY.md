...
| `asc` family | Closest `gpc` family | Google Play API coverage | Status | Notes |
| --- | --- | --- | --- | --- |
| `iap` | `in-app-products` | `inappproducts` | tested | `gpc in-app-products list`, `get`, and `batch-get` cover legacy catalog inspection; `create` applies guarded managed product creation with default-price regional auto-conversion; `patch` applies guarded status, default price, default language, and default listing changes for managed products; `delete` applies guarded managed product deletion. Legacy subscription patches and deletes are rejected; broader regional pricing and tax mutations remain planned. |
