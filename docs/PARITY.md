...
| `crashes` | `vitals` | Play Developer Reporting API | tested | `gpc vitals metric-set get --metric-set crash-rate` and `error-count` cover crash/error metric-set metadata, `gpc vitals metric-set query` can query crash/error metric rows, and `gpc vitals errors issues search` searches grouped crash/ANR/non-fatal issues. Individual report search remains planned. |
