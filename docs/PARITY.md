...
| `subscriptions` | `subscription-offers` | `monetization.subscriptions.basePlans.offers` | tested | `gpc subscription-offers list`, `get`, and `batch-get` cover read-only offer inspection; `list` and `batch-get` support Google wildcard parents. Mutations remain planned. |
