...
| `subscriptions` | `subscription-offers` | `monetization.subscriptions.basePlans.offers` | tested | `gpc subscription-offers list`, `get`, and `batch-get` cover offer inspection; `delete` applies guarded deletion for draft offers; `activate` and `deactivate` apply guarded offer state changes. Broader offer creation and pricing edits remain planned. |
