...
| `iap` | `one-time-products` / `one-time-product-offers` | `monetization.onetimeproducts` | tested | `gpc one-time-products list` and `get` cover modern one-time product inspection, while `purchase-option activate` and `deactivate` apply guarded state changes. `gpc one-time-product-offers list` and `get` inspect discounted and pre-order offers; `activate`, `deactivate`, and `cancel` apply guarded offer state changes. Broader product and offer creation/pricing mutations remain planned. |
| `subscriptions` | `subscriptions` | `monetization.subscriptions`, `basePlans` | tested | `gpc subscriptions list`, `get`, and `batch-get` cover subscription inspection; `base-plan activate` and `deactivate` apply guarded base plan state changes. Broader subscription and price mutations remain planned. |
| `subscriptions` | `subscription-offers` | `monetization.subscriptions.basePlans.offers` | tested | `gpc subscription-offers list`, `get`, and `batch-get` cover offer inspection; `activate` and `deactivate` apply guarded offer state changes. Broader offer creation and pricing edits remain planned. |
...
| `purchases` | `purchases` | `purchases.products`, `purchases.productsv2`, `purchases.subscriptions`, `purchases.subscriptionsv2`, `voidedpurchases` | tested | `gpc purchases product` and `subscription` cover read-only purchase-token status, `gpc purchases product acknowledge` and `consume` apply guarded product purchase mutations, `gpc purchases subscription acknowledge` and `cancel` cover legacy subscription mutations, `gpc purchases subscription revoke` handles guarded full/prorated v2 revocation, and `gpc purchases voided list` covers voided purchase reporting. Item-based revocation remains planned. |
...
| `migrate` | `migrate` | N/A | tested | `gpc migrate supply inspect` inventories fastlane supply metadata directories; `gpc migrate supply convert` emits `gpc metadata apply` compatible JSON for localized listing text. Image and changelog import remains planned. |
