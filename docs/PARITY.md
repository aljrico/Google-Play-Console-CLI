...
| `subscriptions` | `subscription-offers` | `monetization.subscriptions.basePlans.offers` | tested | `gpc subscription-offers list`, `get`, and `batch-get` cover read-only offer inspection, including Google wildcard list parents. Mutations remain planned. |
